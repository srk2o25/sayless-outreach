...
This instance also runs crewzo-webapp. Cadence stays isolated at the OS/Docker level instead of by separate hardware — every rule below exists to keep a LinkedIn incident or a bad script from touching product infra despite sharing the box.

- **Separate Compose project.** Deploy with `docker compose -p cadence up -d` (not the default project name) so Cadence's containers, network, and volumes never collide with or attach to crewzo-webapp's.
...
- **Resource limits**, so Playwright's Chromium can't starve the crewzo-webapp API under load — add to each service in `docker-compose.yml` before first deploy:
  ...
  ```
- **No shared volumes, no shared `.env`.** Cadence's `.env` lives only in this repo's directory on the host.
...
- No inbound rule should ever expose Postgres (5432, or whatever port Cadence's Postgres ends up on) to `0.0.0.0/0`.
